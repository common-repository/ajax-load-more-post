		...
		<td>  
			...
			<div class="cls-row-item">
				<div class="cls-row-item-field">
					<select <?php echo (( isset($fields[$key]["pm"]) && $fields[$key]["pm"] == 1 && $fields["st"]["flag"] != "valid"  )?"disabled='disabled'":""); ?> onchange="<?php echo esc_attr($fields[$key]["onchange"]); ?>" id="<?php echo $this->get_field_id( $key ); ?>" name="<?php echo $this->get_field_name( $key ); ?>" class=" <?php echo esc_attr($fields[$key]["class"]); ?> rplg_field rplg_field_option is_required_<?php echo esc_attr($fields[$key]["is_required"]); ?> <?php echo (( isset($fields[$key]["pm"]) && $fields[$key]["pm"] == 1 && $fields["st"]["flag"] != "valid"  )?"fld_disabled":""); ?>">
						<?php  
...
<?php } else if( $type == "shortcode" ) { ?>	
	<tr>
		...
		<td>
			<div class="cls-row-item">
				<div class="cls-row-item-field">
					<select <?php echo (( isset($fields[$key]["pm"]) && $fields[$key]["pm"] == 1 && $fields["st"]["flag"] != "valid"  )?"disabled='disabled'":""); ?> onchange="<?php echo esc_attr($fields[$key]["onchange"]); ?>" id="id_rplg_<?php echo $key; ?>" name="nm_<?php echo esc_attr($key); ?>" class="<?php echo esc_attr($fields[$key]["class"]); ?> rplg_field rplg_field_option is_required_<?php echo esc_attr($fields[$key]["is_required"]); ?> <?php echo (( isset($fields[$key]["pm"]) && $fields[$key]["pm"] == 1 && $fields["st"]["flag"] != "valid"  )?"fld_disabled":""); ?>">
						<?php  
							foreach( $fields[$key]["options"] as $kw => $value ) {
