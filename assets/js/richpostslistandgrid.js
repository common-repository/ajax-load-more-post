...
function rplg_init_elements(){
	jQuery(document).ready(function($){
		$("#richpostslistandgrid .wea_content  ").each(function(){  
			rplg_ikh_resize_elements($(this).attr("id")); 
		});
	});
} 

function rplg_reload_current_elements(ipc_crid){
	jQuery(document).ready(function($){ 
		rplg_ikh_resize_elements(ipc_crid); 
...
function rplg_ikh_resize_elements(elementId) {
	jQuery(document).ready(function($){
			...
			if(ikh_config_elements["ikh_templates"]=="fit_to_sides") {
				rplg_ikh_fit_to_sides(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_left") {
				rplg_ikh_fixed_width_left(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_center") {
				rplg_ikh_fixed_width_center(ikh_config_elements,root_element);
			}
			
			$(root_element).find(".ik-post-load-more").show();
			$(root_element).find(".rplg-simple-paging").show(); 
	});	
}

function rplg_ikh_fit_to_sides(ikh_config_elements,root_element) {
	...
	jQuery(document).ready(function($){
		...
		if(ikh_main_width<=ikh_item_area_width) {
				...
				$(ikh_element).find(".rplg-simple-paging").removeAttr("style").css("position","relative");	 
		} else {
				...
				for( var ikhw = 60; ikhw > 0; ikhw-- ) {
					...
					if( ikh_main_width >= ikh_items_width ) {
						...
						for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
							...
							var ikh_mrg_lft = 0; 
							if( ikh_j1 == 0 ) { 
								rplg_ikh_set_element_postion(ikh_element,ikh_j1,ikh_first_left,0,ikh_config_elements);
								ikh_mrg_lft = 0;
							} else {
								if( (ikh_j1%ikhw) == 0 ) {
									ikh_last_left = ikh_first_left;
								...
								if( ((ikh_j1-1)%(ikhw)) == 0 && ikhw>1 ) 
									ikh_last_left = ikh_last_left - ikh_margin_left-ikh_border_difference;
								rplg_ikh_set_element_postion(ikh_element,ikh_j1,ikh_last_left,(ikh_num_lines*(ikh_image_height1+ikh_margin_bottom)),ikh_config_elements);	
		...
		rplg_ikh_common_innerelement_load(ikh_element);  
	
	});
}	

function rplg_ikh_fixed_width_left(ikh_config_elements,root_element) {
...
function rplg_ikh_fixed_width_center(ikh_config_elements, root_element) {
	...
	jQuery(document).ready(function($){
		...
		for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
			...
			var ikh_mrg_lft = 0; 
			if( ikh_j1 == 0 ) { 
				rplg_ikh_set_element_postion(ikh_element,ikh_j1,(ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2)),0,ikh_config_elements);
				ikh_mrg_lft = 0;
				ikh_last_left = ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2);
			} else {
				if( (ikh_j1%ikhw) == 0 ) {
...
function rplg_ikh_common_innerelement_load(ikh_element){
	jQuery(document).ready(function($){
		...
		} if($(ikh_element).find(" .rplg-simple-paging").length>0){
			main_final_height = main_final_height + ($(ikh_element).find(".rplg-simple-paging").height()+30);
			var left_bottom_pos = ($(ikh_element).find(" .rplg-simple-paging").width()/2); 
			$(ikh_element).find(" .rplg-simple-paging").css("display","none"); 
			$(ikh_element).find(" .rplg-simple-paging").css("top",pagination_top+"px").css("position","absolute");
			$(ikh_element).find(" .rplg-simple-paging").css("left",(($(ikh_element).width()/2)-left_bottom_pos)+"px");
			$(ikh_element).css("display","block"); 
			$(ikh_element).find(" .item-posts-wrap").css("height",main_final_height+"px");
			$(ikh_element).css("height",main_final_height+"px");
			$(ikh_element).find(" .rplg-simple-paging").css("position","absolute");
			$(ikh_element).find(" .rplg-simple-paging").css("display","block");
		} else {
		...
		var ikh_main_width = parseInt($(ikh_element).width());      
		if(ikh_main_width<=ikh_item_area_width) {
			$(ikh_element).find(" .rplg-simple-paging").css("position","relative").removeAttr("style").show();
			$(ikh_element).find(" .ik-post-load-more").css("position","relative").removeAttr("style").show();	
		    $(ikh_element).find(" .rplg-simple-paging").css("position","relative");
...
function rplg_ikh_set_element_postion(ikh_element,ikh_ele,ikh_left,ikh_top,ikh_config_elements_1){			 
...
function rplg_loadMorePosts(category_id,limit,elementId,total,request_obj){
	if(flg_v1==1) return;
	jQuery(document).ready(function($){ 
			var root_element = $("#"+elementId).parent();
			category_id = $(root_element).find(".ik-drp-post-category").val();
			...
			request_obj.action = 'rplg_getMorePosts';
			request_obj.security = richpostslistandgrid.rplg_security;
			...
			$.ajax({
				...
				success:function(data) {     
					rplg_printData(elementId,data,"loadmore");
				},error: function(errorThrown){ console.log(errorThrown);}
			});
	});
}
function rplg_fillPosts(elementId,category_id,request_obj,flg_pr){
	if(flg_v1==1) return;
 	jQuery(document).ready(function($){
			...
			category_id = $(".ik-drp-post-category").val();
			if((category_id==='undefined' || category_id==='all')) category_id = 0;
			if((post_search_text==='undefined')) post_search_text = "";
 			flg_v1 = 1;
			
			request_obj.action = 'rplg_getPosts';
			request_obj.security = richpostslistandgrid.rplg_security;
			request_obj.category_id_default = category_id;
			request_obj.post_search_text = post_search_text; 
			//security: richpostslistandgrid.rplg_security,
		 	$.ajax({
				...
				data: request_obj,
				success:function(data) { 
					rplg_printData(elementId,data,"fillpost"); 
				},error: function(errorThrown){ console.log(errorThrown);}
			});   
	});	 
}
function rplg_printData(elementId,data,flg){
	jQuery(document).ready(function($){
		...
		if(flg=="loadmore"){  
			...
			if ($("#"+elementId).hasClass("simple_numeric_pagination") || $("#"+elementId).hasClass("next_and_previous_links")) {			
				...
				$(root_element).find(".item-posts").find(".rplg-simple-paging").remove();
		...
		rplg_reload_current_elements(elementId);
		flg_v1 = 0;	
	});	  
	
}
var flg_ms_hover = 0;
function rplg_pr_item_image_mousehover(ob_pii){ 
...
function rplg_pr_item_image_mouseout(ob_pii){
	jQuery(document).ready(function($){ 
...
function rplg_cat_tab_ms_out(ob_ms_eff){
	jQuery(document).ready(function($){ 
		$(ob_ms_eff).removeClass("pn-active-bg"); 	
	});
}
function rplg_cat_tab_ms_hover(ob_ms_eff){
	jQuery(document).ready(function($){ 
		$(ob_ms_eff).addClass("pn-active-bg"); 	
	});
} 

function rplg_init_accordion(){
	jQuery(document).ready(function($){
		...
		$(".ik-post-search-text,.ik-drp-post-category").live("keyup",function(e){
			if(e.keyCode==13){
		...
		setInterval(function(){  
			 $("#richpostslistandgrid .wea_content .pn-active").each(function(){ 
				...
				$(ikh_element).find(".ikh-post-item").each(function(){
					 if( $(this).height() <= 50 ){
						rplg_ikh_resize_elements(attr_id);  
					 }
				});  
			});
		},500);
...
if ( window.addEventListener ) { 
	window.addEventListener( "load", rplg_init_accordion, false );
}
else 
{    
	if ( window.attachEvent ) { 
		  window.attachEvent( "onload", rplg_init_accordion );
	} else {
		 if ( window.onLoad ) {
		   window.onload = rplg_init_accordion;
		 }
	}	 
}